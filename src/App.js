...
function App() {
  const [homeScore, homeAddScore] = useState(0);
  const [awayScore, awayAddScore] = useState(0);

  const homeTouchdown = event => homeAddScore(homeScore + 7)
  const homeFieldGoal = event => homeAddScore(homeScore + 3)

  const awayTouchdown = event => awayAddScore(awayScore + 7)
  const awayFieldGoal = event => awayAddScore(awayScore + 3)
      ...
      <section className="scoreboard">
        <div className="topRow">
          <div className="home">
            ...
            {/* DONE STEP 3 - We need to change the hardcoded values in these divs to accept dynamic values from our state. */}
